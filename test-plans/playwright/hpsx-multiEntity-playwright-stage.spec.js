...
// Load random user from CSV file (only users 1-4900 which are created)
const loadRandomUser = () => {
    const csvPath = path.join(__dirname, '../../user-files/hpsxpw-users-6000.csv');
    const csvContent = fs.readFileSync(csvPath, 'utf-8');
    const lines = csvContent.trim().split('\n');
    // Only use first 4900 users (already created)
    const availableLines = lines.slice(0, 4900);
...
test('Salesforce Recording - Wed Jan 07 2026 15:14:02 GMT+0530 (India Standard Time)', async ({
    ...
    await page.goto(
        'https://stagecom.test1.my.pc-rnd.salesforce.com/',
    );
    await delay(1);
    
    // Load user from CSV or use environment variables
    let username, password;
    if (process.env.SF_USERNAME || process.env.USERNAME) {
        username = process.env.SF_USERNAME || process.env.USERNAME;
        password = process.env.SF_PASSWORD || process.env.PASSWORD || 'Test1234!';
    } else {
    ...
    // Handle password change screen - navigate directly to home instead
    const currentUrl = page.url();
    console.log('Current URL after login:', currentUrl);
    
    if (currentUrl.includes('ChangePassword')) {
        console.log('Password change screen detected - navigating to home directly');
        // Navigate directly to Lightning Experience home
        await page.goto('https://stagecom.test1.my.pc-rnd.salesforce.com/lightning/page/home');
        await page.waitForLoadState('domcontentloaded', { timeout: 60000 });
        await delay(3000);
    }
